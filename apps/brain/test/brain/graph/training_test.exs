defmodule Brain.Graph.TrainingTest do
  ...
  describe "extract_intent_priors/0" do
    test "builds transition matrix from conversation_graph topics" do
      ...
      # Seeds have greeting -> weather.query transition
      if Map.has_key?(priors, "greeting") do
        transitions = priors["greeting"]
  ...
  describe "apply_intent_priors/4" do
    test "boosts scores based on prior transitions" do
      scores = [{"weather.query", 0.8}, {"greeting", 0.3}, {"unknown", 0.1}]
      priors = %{"greeting" => %{"weather.query" => 5, "greeting" => 1}}

      boosted = Brain.Graph.Training.apply_intent_priors(scores, "greeting", priors, weight: 0.3)
    ...
    test "returns original scores when prev_intent not in priors" do
      scores = [{"weather.query", 0.8}, {"greeting", 0.3}]
      priors = %{}
