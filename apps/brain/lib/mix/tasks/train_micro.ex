defmodule Mix.Tasks.TrainMicro do
  ...
  def run(args) do
    ...
    {opts, _, _} =
      OptionParser.parse(args,
        switches: [only: :string, list: :boolean, verbose: :boolean],
    ...
    if opts[:list] do
      ...
      output_dir = Brain.priv_path("ml_models/micro")
      File.mkdir_p!(output_dir)

      results =
        Enum.map(names, fn name ->
          train_classifier(name, output_dir, opts[:verbose] || false)
        end)
  ...
  defp train_classifier(name, output_dir, verbose) do
  ...
  defp list_classifiers do
    ...
    Enum.each(@classifier_names, fn name ->
      data_path = data_file_path(name)
      model_path = Path.join(Brain.priv_path("ml_models/micro"), "#{name}.term")
