defmodule Brain.Knowledge.ReviewQueue do
  ...
  alias Brain.Knowledge.{HtmlProcessor, SourceReliability, Types}
  ...
  defp default_persistence_path, do: Brain.priv_path("data/review_queue.term")
  ...
  defp broadcast_update(event, data) do
    if Process.whereis(Brain.PubSub) do
      Phoenix.PubSub.broadcast(Brain.PubSub, "knowledge:review", {event, data})
    end
  rescue
    _ -> :ok
  end
