defmodule Brain.Analysis.ContextualEntityInferrer do
  ...
  defp atlas_context_signal(entity, narrowed_type) do
    if Brain.AtlasIntegration.available?() do
      try do
        context_results = Brain.Graph.Reader.entity_context([entity], depth: 2)

        case context_results do
          [%{neighbors: neighbors}] when is_list(neighbors) and length(neighbors) > 0 ->
            type_matches = Enum.count(neighbors, fn n ->
              neighbor_label = Map.get(n, :label, "")
              neighbor_name = get_in(n, [:properties, "entity_type"]) ||
                              get_in(n, [:properties, "name"]) || ""

              String.downcase(neighbor_label) == narrowed_type or
                String.downcase(neighbor_name) == narrowed_type or
                TypeHierarchy.is_a?(String.downcase(neighbor_label), narrowed_type)
            end)
