defmodule Brain.AtlasIntegration do
  ...
  def persist_belief(%Brain.Epistemic.Types.Belief{} = belief) do
    async(fn ->
      ...
      case Atlas.Repo.get(Atlas.Schemas.Belief, belief.id) do
        # The id goes on the struct, not through the changeset:
        # `Atlas.Schemas.Belief.changeset/2` does not cast `:id`
        # (`belief.ex:33-38`), so passing it in `attrs` dropped it and the
        # insert autogenerated a different one. The brain then never found the
        # belief it had just written, and every later persist inserted another
        # copy.
        nil ->
          %Atlas.Schemas.Belief{id: belief.id}
  ...
  Returns `{:ok, vertex}` when the graph holds one, `:not_found` when the
  query succeeded and matched nothing, and `{:error, reason}` when the lookup
  could not be performed.

  Those last two are different answers and must stay different. This function
  used to report both as `:not_found`, so a connection that refused us, a
  sandbox ownership violation or a malformed query all read as "the graph does
  not know about this". A caller cannot recover from what it cannot see: one
  such `:not_found` was cached as a fact and answered every later reader
  (see `.claude/findings/2026-09-19-test-failure-trace.md`).
  """
  ...
  # Same contract as `find_node/3`: a failed synonym lookup is an error, not
  # an absence. The first synonym that resolves wins; a failure on any of them
  # is reported rather than read as "no synonym matched".
  defp find_node_via_synonyms(graph, label, name) do
    synonyms = Brain.Lexicon.synonyms(to_string(name))

    Enum.reduce_while(Enum.take(synonyms, 5), :not_found, fn syn, acc ->
      escaped = String.replace(syn, "'", "\\'")
      query = "MATCH (n:#{label}) WHERE n.name = '#{escaped}' RETURN n"

      case Atlas.Graph.cypher(graph, query) do
        {:ok, [[%Atlas.Graph.Types.Vertex{} = v] | _]} -> {:halt, {:ok, v}}
        {:ok, _} -> {:cont, acc}
  ...
  @doc """
  Find or create a graph node.

  Returns `{:ok, vertex}` for an existing or newly created node, and
  `{:error, reason}` when the lookup itself failed — in that case nothing is
  created, because "we could not ask" is not "it is not there", and creating
  on a failed lookup is how duplicates of the same node appear.
  """
