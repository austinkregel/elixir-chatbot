  ...
  def learn_from_input(persona_name, input, opts \\ []) do
    ...
    entity_opts =
      if discourse || speech_act do
        ...
        discourse_result =
          try do
            Brain.Analysis.DiscourseAnalyzer.analyze(input, [])
          rescue
            _ -> nil
          catch
            _ -> nil
        ...
        speech_act_result =
          try do
            Brain.Analysis.SpeechActClassifier.classify(input)
          rescue
            _ -> nil
          catch
            _ -> nil
    ...
    entities = Brain.ML.EntityExtractor.extract_entities(input, entity_opts)
