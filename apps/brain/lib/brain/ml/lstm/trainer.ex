defmodule Brain.ML.LSTM.Trainer do
  ...
  alias LSTM.{SharedEncoder, IntentHead, NERHead, POSHead}
  ...
  @doc "Load a trained intent classification model.\n"
  ...
  @doc "Load a trained joint model.\n"
  ...
  @doc "Classify intent and extract entities using a joint model.\n"
  ...
  defp joint_train_loop(
    ...
    {enc_opt_init, enc_opt_update} = Optimizers.adam(learning_rate: config.learning_rate)
    {int_opt_init, int_opt_update} = Optimizers.adam(learning_rate: config.learning_rate)
    {ner_opt_init, ner_opt_update} = Optimizers.adam(learning_rate: config.learning_rate)
    ...
    {final_state.encoder_params, final_state.intent_params, final_state.ner_params,
     final_state.metrics}
  end

  defp joint_train_epoch(
         encoder,
         intent_head,
         ner_head,
         train_data,
      ...
      if batch_indices == [] do
        ...
        {loss, {enc_grads, int_grads, ner_grads}} =
          compute_joint_loss_and_grads(
            ...
            batch_bio_tags,
            batch_masks,
            acc_state.encoder_params,
            acc_state.intent_params,
            acc_state.ner_params,
            config
          )
  ...
  defp compute_joint_loss_and_grads(
         encoder,
         intent_head,
         ner_head,
         inputs,
  ...
  @doc "Load a trained multi-task model.\n"
  ...
  @doc "Full analysis using multi-task model: intent, entities, and POS tags.\n"
  def analyze_multitask(text, model) do
    ...
    intent_labels =
      get_sorted_labels(model.vocabularies.idx_to_intent, model.vocabularies.intent_to_idx)
  ...
  defp multitask_train_loop(
    ...
    {int_opt_init, int_opt_update} = Optimizers.adam(learning_rate: config.learning_rate)
    {ner_opt_init, ner_opt_update} = Optimizers.adam(learning_rate: config.learning_rate)
    {pos_opt_init, pos_opt_update} = Optimizers.adam(learning_rate: config.learning_rate)
    ...
    {final_state.encoder_params, final_state.intent_params, final_state.ner_params,
     final_state.pos_params, final_state.metrics}
  end

  defp multitask_train_epoch(
         encoder,
         intent_head,
         ner_head,
         pos_head,
    ...
    Enum.reduce(0..(num_batches - 1), {0.0, state}, fn batch_idx, {total_loss, acc_state} ->
      ...
      if batch_indices == [] do
        ...
        batch_inputs =
          batch_indices |> Enum.map(&Enum.at(train_data.inputs, &1)) |> Nx.tensor(type: :s64)

        batch_intents =
          batch_indices |> Enum.map(&Enum.at(train_data.intents, &1)) |> Nx.tensor(type: :s64)

        batch_bio =
          batch_indices |> Enum.map(&Enum.at(train_data.bio_tags, &1)) |> Nx.tensor(type: :s64)

        batch_pos =
          batch_indices |> Enum.map(&Enum.at(train_data.pos_tags, &1)) |> Nx.tensor(type: :s64)

        batch_masks =
          batch_indices |> Enum.map(&Enum.at(train_data.masks, &1)) |> Nx.tensor(type: :f32)
  ...
  defp train_loop(
    ...
    {encoder_opt_init, encoder_opt_update} =
      Optimizers.adam(learning_rate: config.learning_rate)

    {intent_opt_init, intent_opt_update} =
      Optimizers.adam(learning_rate: config.learning_rate)
  ...
  defp check_early_stopping(state, val_loss, config, epoch_metrics) do
    ...
    if val_loss < state.best_val_loss do
      {true, %{state | best_val_loss: val_loss, patience_counter: 0, metrics: new_metrics}}
    else
  ...
  defp log_epoch_progress(epoch, total_epochs, :complete, metrics) do
    ...
    Logger.info(
      "Epoch #{epoch}/#{total_epochs} #{bar} done in #{duration} | train_loss: #{train_loss} | val_loss: #{val_loss}"
    )
  end
  ...
  defp log_epoch_progress_with_acc(epoch, total_epochs, metrics) do
    ...
    Logger.info(
      "Epoch #{epoch}/#{total_epochs} #{bar} done in #{duration} | loss: #{train_loss}/#{val_loss} | acc: #{train_acc}%/#{val_acc}%"
    )
  end
