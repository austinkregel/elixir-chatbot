defmodule Fleet.EnsignTest do
  @moduledoc """
  Proves the ensign mechanics: spawn → order → ACK → autonomous tick →
  ...
  # We commission ensigns into the running CrewSupervisor and retire them after
  # each test for isolation.
  setup do
    test_pid = self()
    handler_id = "test-ensign-#{System.unique_integer([:positive])}"
  ...
  test "mailbox stays responsive while cognition is in flight", %{soul: soul} do
    ...
    Ensign.order(id, %Order{id: "o2", reply_to: self(), directive: "long task", dry_run: true})
    ...
    {micros, status} = :timer.tc(fn -> Ensign.status(id) end)
    assert status.agent_id == id
    assert micros < 100_000
  end

  test "unknown soul id leaves the ensign alive but not ready" do
    {:ok, _pid, id} = CrewSupervisor.start_ensign(soul_id: "no-such-soul", tick_interval: 50)

    assert_receive {:tele, :spawned, _, _}
    assert_receive {:tele, :soul_hydrate_failed, _, _}
    refute Ensign.ready?(id)
    # Still addressable and responsive.
    assert Ensign.status(id).soul_loaded == false
  end
  ...
  # Proves the ensign runs REAL cognition through a world where JJ-7 resides, so
  # the acting soul's constitution steers generation (not the generic default).
  @tag :integration
  test "ensign runs real cognition through a world where JJ-7 resides" do
    ...
    # the ensign's cognition soul-steered rather than the generic default.
    ...
    assert is_integer(dt)
    assert Ensign.status(id).assignment_status == "completed"
  end
end
