defmodule World.TypeInferrer do
  ...
  defp serialize_ets_entry({{world_id, type}, data}) when is_binary(world_id) and is_binary(type) do
    {"#{world_id}::#{type}", data}
  end
  ...
  # Convert string keys back to tuple keys when importing
  ...
  def import_learned_data(data) when is_map(data) do
    ...
    # Import patterns - handle both atom and string keys (from JSON)
    ...
    # Import co-occurrences - handle both atom and string keys (from JSON)
  ...
  defp match_learned_patterns(context_features, world_id) do
    ...
    try do
      :ets.tab2list(@ets_patterns)
      |> Enum.filter(fn
        # Expected format: {{world_id, type}, patterns}
        {{w, _type}, _patterns} when is_binary(w) -> w == world_id
        # Legacy or slot_mappings format - skip these entries
        _ -> false
      end)
  ...
  defp match_cooccurrences(context_tokens, world_id) do
    ...
    if map_size(known_entities) == 0 do
      ...
      try do
        :ets.tab2list(@ets_cooccurrence)
        |> Enum.filter(fn
          # Expected format: {{world_id, type}, cooc_counts}
