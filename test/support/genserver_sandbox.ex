defmodule ChatBot.GenServerSandbox do
  @moduledoc """
  ...
  For most tests that just need clean state but can share global names:
      ...
      setup do
        ChatBot.GenServerSandbox.reset_global_state()
        :ok
      end
  ...
  For edge case tests that need completely isolated GenServer instances:
      ...
      setup do
        {:ok, ctx} = ChatBot.GenServerSandbox.checkout()
        {:ok, ctx}
      end

      test "edge case with custom data", %{sandbox: sandbox} do
        # Start isolated Brain with custom name
        {:ok, brain} = ChatBot.GenServerSandbox.start_isolated(sandbox, ChatBot.Brain, 
  ...
  - `{:via, Registry, {ChatBot.GenServerSandbox.Registry, {Module, sandbox_id}}}`
  ...
  def reset_global_state do
    ...
    if pid = Process.whereis(ChatBot.Brain) do
      try do
        ChatBot.Brain.reset_state(server: pid)
      catch
        _, _ -> :ok
      end
    end
    ...
    if Process.whereis(ChatBot.ML.IntentClassifierSimple) do
      try do
    ...
    if Process.whereis(ChatBot.ML.Gazetteer) do
      try do
  ...
  ## Examples
      ...
      {:ok, pid} = start_isolated(sandbox, ChatBot.MemoryStore)

      # Start with custom child spec
      {:ok, pid} = start_isolated(sandbox, ChatBot.Brain, 
        {ChatBot.Brain, "path/to/artifact.json"})

      # Start Gazetteer with isolated ETS tables
      {:ok, pid} = start_isolated(sandbox, ChatBot.ML.Gazetteer,
        {ChatBot.ML.Gazetteer, [table_prefix: :test_123]})
  """
  ...
  def start_isolated_environment(%{sandbox: sandbox} = ctx) do
    # Core services
    services = [
      {ChatBot.Metrics.Aggregator, ChatBot.Metrics.Aggregator},
      {ChatBot.ML.Gazetteer, {ChatBot.ML.Gazetteer, [table_prefix: sandbox.id]}},
      {ChatBot.Analysis.LearningStore, ChatBot.Analysis.LearningStore},
      {ChatBot.KnowledgeStore, ChatBot.KnowledgeStore},
      {ChatBot.MemoryStore, ChatBot.MemoryStore},
      {ChatBot.Memory.Embedder, ChatBot.Memory.Embedder},
      {ChatBot.Memory.Store, ChatBot.Memory.Store},
      {ChatBot.ML.IntentClassifierSimple, ChatBot.ML.IntentClassifierSimple}
    ]
    ...
    if classifier = Map.get(started, ChatBot.ML.IntentClassifierSimple) do
      try do
    ...
    if gazetteer = Map.get(started, ChatBot.ML.Gazetteer) do
      try do
  ...
  def start_isolated_brain(%{sandbox: sandbox} = ctx, artifact_path \\ "priv/static/demo.echo.json") when is_map(sandbox) do
    ...
    # Start Brain
    brain_spec = {ChatBot.Brain, artifact_path}
    {:ok, brain_pid} = start_isolated(ctx, ChatBot.Brain, brain_spec)
