defmodule Mix.Tasks.ClearKnowledge do
  ...
  defp clear_learned_knowledge do
    ...
    Enum.each(agents, fn agent ->
      knowledge = ChatBot.KnowledgeStore.get_knowledge(agent)
      ...
      if length(counts) > 0 do
        ChatBot.KnowledgeStore.clear(agent)
    ...
    knowledge_dir = Application.get_env(:chat_bot, :knowledge_dir, "priv/knowledge")
  ...
  defp clear_brain_memory do
    ...
    Enum.each(conversations, fn conv ->
      ChatBot.Brain.end_conversation(conv.id)
    end)

    # Clear memory directory
    memory_dir = Application.get_env(:chat_bot, :memory_dir, "priv/memory")
  ...
  defp clear_admin_entities do
    ...
    case ChatBot.ML.Gazetteer.clear_admin_entries() do
      {:ok, count} ->
  ...
  defp reload_gazetteer do
    ...
    case ChatBot.ML.Gazetteer.clear_all() do
      {:ok, count} ->
