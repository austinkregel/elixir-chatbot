defmodule ChatBot.Learning.WorldManager do
  @moduledoc """
  ...
  alias ChatBot.Learning.{TrainingWorld, WorldEvents, WorldMetrics, WorldPersistence}
  ...
  def handle_call({:create_world, name, opts}, _from, state) do
    ...
    ChatBot.ML.Gazetteer.create_world_overlay(world.id)
  ...
  def handle_call({:destroy_world, world_id}, _from, state) do
    case :ets.lookup(@ets_worlds, world_id) do
      [{^world_id, world}] ->
        ...
        ChatBot.ML.Gazetteer.destroy_world_overlay(world_id)
  ...
  def handle_call({:promote_candidate, world_id, candidate_value, entity_type}, _from, state) do
    ...
    result = ChatBot.ML.Gazetteer.add_to_world(world_id, candidate_value, entity_type)
    ...
    case :ets.lookup(@ets_worlds, world_id) do
      [{^world_id, world}] ->
        if world.mode == :persistent do
          ...
          overlay = ChatBot.ML.Gazetteer.get_world_overlay(world_id)
  ...
  def handle_call({:merge, source_id, target_id, opts}, _from, state) do
    ...
    with {:ok, _source} <- get(source_id),
      ...
      source_overlay = ChatBot.ML.Gazetteer.get_world_overlay(source_id)
      ...
      if require_review and length(entities_to_merge) > 0 do
        ...
        Enum.each(entities_to_merge, fn {_key, info} ->
          ChatBot.ML.Gazetteer.add_to_world(
            target_id,
            info.value,
            info.entity_type,
